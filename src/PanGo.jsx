    ...
    try{
      ...
      for(const fecha of diasConPedido){
        ...
        dia.items.forEach(i=>{
          mpItems.push({
            id:i.product.id,
            title:`${i.product.name}${i.weight?` ${PESOS[i.weight]}`:""} — ${fd?.diaFull||fecha}`,
            quantity:i.qty,
            unit_price:i.price,
          });
        });
      }
      // Llamar al endpoint serverless /api/create-preference
      const resp = await fetch("/api/create-preference",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          items:mpItems,
          payer:user?{email:user.email,name:user.name}:{},
        }),
      });
      if(resp.ok){
        ...
        // Si no hay token configurado, ir a pantalla demo
        setScreen("payment");
      }
    }catch(e){
      // Fallback demo
            ...
            <button onClick={async()=>{
              ...
              try{
                ...
                if(resp.ok){
                  const data=await resp.json();
                  if(data.init_point) window.location.href=data.init_point;
                  else showToast("Configura MP_ACCESS_TOKEN en Vercel");
                } else showToast("Configura MP_ACCESS_TOKEN en Vercel → pago simulado activo");
              }catch{showToast("Backend MP no configurado → usa simulación");}
              finally{setLoading(false);}
            }} style={{display:"block",width:"100%",background:C.noir,color:C.creme,padding:"15px 0",
              border:"none",fontSize:13,fontWeight:600,letterSpacing:2,textTransform:"uppercase",
  ...
  return(
    <div style={{background:C.bg,display:"flex",flexDirection:"column"}}>
      <div style={{height:230,overflow:"hidden",position:"relative",background:C.noir}}>
        <ProdImg id={product.id} height={230}/>
  ...
  return(
    <div style={{background:"#fff",display:"flex",alignItems:"center",gap:0,borderBottom:`1px solid ${C.warm}`}}>
      ...
      <div style={{width:90,height:90,flexShrink:0,overflow:"hidden",background:C.noir}}>
        <ProdImg id={product.id} height={90}/>
      </div>
      {/* Info */}
    ...
    <div>
      <div style={{minHeight:"92vh",display:"flex",flexDirection:"column",
        ...
        <img src="https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&w=1600&q=85"
          ...
          onError={e=>{e.currentTarget.parentElement.style.background="#1A1208";e.currentTarget.style.display="none";}}/>
